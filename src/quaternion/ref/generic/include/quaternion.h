...
/**
 * @brief Add two algebra elements
 *
 * Sets `res = a + b` without normalizing the result.
 *
 * @param res Output algebra element
...
/**
 * @brief Subtract two algebra elements
 *
 * Sets `res = a - b` without normalizing the result.
 *
 * @param res Output algebra element
...
/**
 * @brief Multiply an algebra element by an integer scalar
 *
 * Sets `res = scalar * elem` without normalizing the result.
 *
 * @param res Output algebra element
 * @param scalar Integer scalar
 * @param elem Input algebra element
 */
...
/**
 * @brief Set the norm of a left ideal
 *
 * Ensures `lideal->norm` matches the norm derived from its lattice and parent order.
 *
 * @param lideal In/out left ideal with lattice and parent order already initialized
 */
...
/**
 * @brief Multiply a left ideal by an algebra element
 *
 * Computes the left ideal `lideal * alpha` (with multiplication on the right).
 *
 * @param product Output ideal
 * @param lideal Input left ideal
 * @param alpha Algebra element
 * @param alg Quaternion algebra descriptor
 */
...
/**
 * @brief Test if an order is maximal
 *
 * Validates that the discriminant of `order` equals the defining prime of the quaternion algebra.
 *
 * @param order Candidate order of the algebra
 * @param alg Quaternion algebra descriptor
 * @return 1 if the order is maximal (assuming it is an order), 0 otherwise
 */
